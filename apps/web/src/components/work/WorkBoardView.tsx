...
export function WorkBoardView({
  ...
  async function handleConfirmBlocker(e: React.FormEvent) {
    e.preventDefault();
    if (!selectedTaskForBlock || isSubmitting) return;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        ...
        <div className="flex items-center gap-2 flex-wrap w-full sm:w-auto justify-start sm:justify-end">
          ...
          <button
            ...
            className={`inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 text-xs font-semibold rounded-xl shadow-2xs transition-colors shrink-0 ${
              isAILoading ? "opacity-60 cursor-not-allowed pointer-events-none" : "cursor-pointer"
            }`}
          >
            {isAILoading ? (
          ...
          <button
            ...
            className="inline-flex items-center gap-1 px-3.5 py-1.5 bg-slate-900 hover:bg-black text-white text-xs font-semibold rounded-xl shadow-xs active:scale-[0.98] transition-all shrink-0 cursor-pointer"
          >
      ...
      {viewMode === "kanban" && (
        <div className="space-y-2">
          ...
          <div className="xl:hidden flex items-center justify-end gap-1.5 text-[11px] font-medium text-slate-500 px-1 select-none">
                                ...
                                {activeMenuTaskId === task.id && (
                                  ...
                                  >
                                    ...
                                    >
                                      ...
                                      <span>{processingTaskId === task.id ? "Mengarsipkan..." : "Arsipkan Task"}</span>
                                    </button>
                                    ...
                                    >
                                      ...
                                      <span>{processingTaskId === task.id ? "Menghapus..." : "Hapus Task"}</span>
                                    </button>
                                  </div>
                                )}
                              </div>
                ...
                <tr key={t.id} className="hover:bg-slate-50/70 transition-colors">
                  ...
                  <td className="py-2.5 px-4 text-right">
                    <div className="flex items-center justify-end gap-1.5">
                      ...
                      <button
                        type="button"
                        disabled={processingTaskId === t.id}
                        onClick={() => handleArchiveTask(t.id, t.title)}
                        className={`text-slate-600 hover:text-slate-900 px-2 py-1 rounded hover:bg-slate-100 flex items-center gap-1 ${
                          processingTaskId === t.id ? "opacity-60 pointer-events-none" : ""
                        }`}
                      >
                        {processingTaskId === t.id && <Loader2 className="w-3 h-3 animate-spin text-slate-500" />}
                        <span>Arsip</span>
                      </button>
                      <button
                        type="button"
                        ...
                        className={`text-rose-600 hover:text-rose-700 px-2 py-1 rounded hover:bg-rose-50 flex items-center gap-1 ${
                          processingTaskId === t.id ? "opacity-60 pointer-events-none" : ""
                        }`}
                      >
                        {processingTaskId === t.id && <Loader2 className="w-3 h-3 animate-spin text-rose-500" />}
                        <span>Hapus</span>
                      </button>
                    </div>
                  </td>
      ...
      {isTaskModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div
            className="bg-white rounded-2xl border border-slate-200 shadow-xl max-w-lg w-full p-4 sm:p-6 space-y-4 max-h-[85dvh] sm:max-h-[90vh] overflow-y-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="font-bold text-slate-900 text-sm sm:text-base">Buat Task Baru</h3>
              <button
                type="button"
            ...
            <form onSubmit={handleCreateFullTask} className="space-y-3">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Keterangan teknis tugas, instruksi pengerjaan, atau catatan..."
                  className="w-full px-3 py-2.5 text-xs font-mono border border-slate-200 rounded-xl bg-slate-50 min-h-[130px] sm:min-h-[160px] resize-y focus:bg-white focus:outline-none focus:ring-1 focus:ring-slate-400 leading-relaxed text-slate-800"
                />
              </div>
              ...
              <div className="pt-2 flex items-center justify-end gap-2 border-t border-slate-100">
                ...
                <button
                  ...
                  className={`px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold text-xs rounded-lg shadow-xs transition-all flex items-center gap-1.5 ${
                    isSubmitting ? "opacity-60 cursor-not-allowed pointer-events-none" : "cursor-pointer"
                  }`}
                >
                  {isSubmitting && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
                  <span>{isSubmitting ? "Menyimpan..." : "Simpan Task"}</span>
                </button>
              </div>
            </form>
          </div>
            ...
            <form onSubmit={handleConfirmBlocker} className="space-y-3">
              ...
              <div className="pt-2 flex items-center justify-end gap-2 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  disabled={isSubmitting}
                  className={`px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white font-semibold text-xs rounded-lg shadow-xs transition-all flex items-center gap-1.5 ${
                    isSubmitting ? "opacity-60 cursor-not-allowed pointer-events-none" : "cursor-pointer"
                  }`}
                >
                  {isSubmitting && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
                  <span>{isSubmitting ? "Menyimpan..." : "Konfirmasi Status Blocked"}</span>
                </button>
              </div>
            </form>
          </div>
                    ...
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <button
                          ...
                          className={`px-2 py-1 text-[11px] font-semibold text-blue-600 hover:text-blue-700 hover:bg-blue-50 rounded-lg transition-colors flex items-center gap-1 ${
                            processingTaskId === task.id ? "opacity-60 cursor-not-allowed pointer-events-none" : "cursor-pointer"
                          }`}
                        ...
                        >
                          ...
                          <span>{processingTaskId === task.id ? "Memulihkan..." : "Pulihkan"}</span>
                        </button>
                        <button
                          type="button"
                          disabled={processingTaskId === task.id}
