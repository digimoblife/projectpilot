...
export interface CommunicationReportsViewProps {
  projectId: string;
  embedded?: boolean;
}

export function CommunicationReportsView({
  projectId,
  embedded = false,
}: CommunicationReportsViewProps) {
  const { token } = useAuth();
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="inline-flex items-center whitespace-nowrap shrink-0 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200">
      ...
      {embedded && (
        <div className="flex items-center justify-between gap-3">
          <div>
            <h3 className="text-sm font-bold text-slate-900">Laporan Status Proyek</h3>
            <p className="text-xs text-slate-500">
              Penyusunan dan review laporan berkala berbasis bukti proyek (evidence-grounded).
            </p>
          </div>
                ...
                <div>
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    <span className="inline-flex items-center gap-1 font-mono text-xs font-bold px-2 py-0.5 rounded bg-slate-100 text-slate-800 shrink-0 whitespace-nowrap">
                      <span>{selectedReport.report_key}</span>
                      <span className="text-slate-500 font-medium">v{selectedReport.version}</span>
                    </span>
                    <span
